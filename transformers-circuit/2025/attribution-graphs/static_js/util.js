window.util = (function () {
  ...
  async function getFile(path) {
    ...
    if (path.startsWith('/')) {
      // Go up one level from static_js for paths starting with /
      var root = window.location.href.includes('/static_js/') 
        ? window.location.href.split('/static_js/')[0] + '/static_js/..'  
        : window.location.href.split(/[^/]+\.html/)[0].replace(/\/$/, '')                    
      path = root + path
    }
