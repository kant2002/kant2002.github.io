...
як ви бачите, ми маємо лише рядки, і як спеціальний випадок `bool` значення. Насправді навіть "true"/"false" значення це рядки, але операції порівняння будуть працювати із `true`, або із `"true"` і тому краще важати що це такий спеціальний тип.
...
де `PkgMyNugetSdk` це шлях де буде лежати розпакований Nuget пакет `MyNugetSdk`. Назва властивості аналогічна назві яку би згенерував MSBuild якщо для пакету була би встановлені метадані `GeneratePathProperty=true`. Трішки більше можна почитати [тут][sdk-project-files]
